...
// Testet zwei 2D-Sphären ohne Überschneidung
...
// Testet eine 3D-Sphärenüberschneidung in z-Richtung mit expliziter Distanzbetrachtung
TEST(SPHERE, Intersects3dfWithSphere_1) {
  Sphere3df sphere1 = { {0.0, 0.0, 0.0}, 1.0 };
  Sphere3df sphere2 = { {1.5, 0.0, 0.0}, 1.0 };

  EXPECT_TRUE( sphere1.intersects(sphere2) );
}

// Testet den Randfall, dass ein Punkt auf der Oberfläche noch als "inside" gilt
TEST(SPHERE, InsideOnSurface) {
  Sphere3df sphere = { {0.0, 0.0, 0.0}, 1.0 };

  EXPECT_TRUE( sphere.inside( Vector3df{1.0f, 0.0f, 0.0f} ) );
}

// Testet, dass ein Punkt außerhalb der Sphäre korrekt nicht als "inside" gilt
TEST(SPHERE, NotInsideOutside) {
...
// Prüft, dass ein Punkt innerhalb der Kugel noch als "inside" erkannt wird
...
// Prüft, dass ein Punkt außerhalb der Kugel nicht als "inside" gilt
