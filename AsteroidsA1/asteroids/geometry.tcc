#include "geometry.h"
// Konstruktor für AxisAlignedBoundingBox
// Initialisiert die AABB mit einem Mittelpunkt und den halben Kantenlängen
...
// Prüft ob diese AABB mit einer anderen AABB überlappt
// Nutzt Separating Axis Theorem (SAT) für alle Achsen
template <class FLOAT, size_t N>
...
// Prüft ob diese AABB von einem Ray geschnitten wird
// Nutzt die Slab-Methode: für jede Achse den Eintritte- und Austrittsparameter berechnen
template <class FLOAT, size_t N>
...
// Prüft ob diese AABB mit einer moving AABB (in gegebener Richtung) überlappt
// Konvertiert zu einem Ray-AABB Test mit erweiterter AABB
template <class FLOAT, size_t N>
...
// Berechnet den Kollisionsvektor (Normale) für eine moving AABB
// Gibt die Normale der Fläche zurück, die zuerst getroffen wird
// Rückgabe: Nullvektor wenn keine Kollision auftritt
template <class FLOAT, size_t N>
...
// Konstruktor für Sphäre
// Initialisiert die Sphäre mit einem Mittelpunkt und einem Radius
template <class FLOAT, size_t N>
...
// Berechnet den Schnittpunkt eines Rays mit dieser Sphäre
// Löst die quadratische Gleichung: (ray.origin + t * ray.direction - center)² = radius²
// Berücksichtigt Fälle wo der Ray-Ursprung innerhalb oder außerhalb der Sphäre liegt
// Rückgabe: t-Parameter für Schnittpunkt, oder 0 falls kein Schnitt
template <class FLOAT, size_t N>
...
// Prüft ob ein Ray diese Sphäre schneidet und berechnet Schnittinformationen
// Setzt den Schnittpunkt, die Normale und den t-Parameter im context
// Berücksichtigt ob der Ray-Ursprung innerhalb der Sphäre liegt
template <class FLOAT, size_t N>
...
//Ergänzung zu der Aufgabe 
// Prüft ob diese Sphäre mit einer anderen Sphäre überlappt
// Zwei Sphären schneiden sich wenn der Abstand ihrer Mittelpunkte <= Summe ihrer Radien
template <class FLOAT, size_t N>
bool Sphere<FLOAT,N>::intersects(Sphere<FLOAT, N> sphere) const {
  Vector<FLOAT, N> distance = center - sphere.center;
  FLOAT dist_length = distance.length();
  return dist_length <= radius + sphere.radius;
}

// Prüft ob ein Punkt inside dieser Sphäre oder auf ihrer Oberfläche liegt
...
// Konstruktor für Dreieck mit einer gemeinsamen Normale für alle Vertices
template <class FLOAT, size_t N>
...
// Konstruktor für Dreieck - die Normale wird aus dem Kreuzprodukt der Kantenvektoren berechnet
// Die Normale zeigt weg von der Dreiecksoberfläche (bei clockwise Anordnung a,b,c)
template <class FLOAT, size_t N>
...
// Prüft ob dieser Triangle mit einem Ray überlappt und speichert Schnittinformationen im context
template <class FLOAT, size_t N>
...
// Prüft ob ein Ray dieses Dreieck schneidet (Möller-Trumbore Algorithmus)
// Berechnet den Schnittpunkt p und die baryzentrischen Koordinaten u,v
// t ist der Parameter mit: p = ray.origin + t * ray.direction
template <class FLOAT, size_t N>
...
// Berechnet die Richtung eines refraktierten Rays
// Nutzt Snells Gesetz: n1 * sin(θ1) = n2 * sin(θ2)
// Rückgabe: true bei erfolgreicher Refraktion, false bei Totalreflexion
template <class FLOAT, size_t N>
