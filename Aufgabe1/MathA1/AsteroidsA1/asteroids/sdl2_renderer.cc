...
void SDL2Renderer::renderSpaceship(Vector2df position, float angle) {
    // ÄNDERUNG: Raumschiff so zeichnen, dass die Spitze am Ursprung liegt
    static std::array<SDL_Point, 5> ship_points{SDL_Point{0, 0},
                                              SDL_Point{-14, -8},
                                              SDL_Point{-8, 0},
                                              SDL_Point{-14, 8},
                                              SDL_Point{0, 0}};
  ...
  // ÄNDERUNG: Raumschiff in Cyan Farbe zeichnen
...
void SDL2Renderer::render(Spaceship * ship) {
  // ÄNDERUNG: Triebflamme als einfache Linie darstellen
  ...
  if (! ship->is_in_hyperspace()) {
    if (ship->is_accelerating()) {
      ...
      for (size_t i = 0; i < points.size(); i++) {
       ...
       // ÄNDERUNG: Triebflamme in Orange/Rot Farbe zeichnen
...
void SDL2Renderer::render(Saucer * saucer) {
  // ÄNDERUNG: UFO als Rakete mit festem Durchmesser darstellen
  static SDL_Point rocket_points[] = {
    {0, -20},   // Spitze
    {-8, -8},
    {-6, 10},
    {-12, 14},
    ...
    {6, 10},
    {8, -8},
    {0, -20}   // zurück zur Spitze
  };
  std::array<SDL_Point, std::span{rocket_points}.size()> points;

  Vector2df position = saucer->get_position();
  float scale = 1.0f; // konstante Größe
  for (size_t i = 0; i < points.size(); i++) {
    float x = rocket_points[i].x;
    float y = rocket_points[i].y;
    points[i].x = scale * x + position[0];
    points[i].y = scale * y + position[1];
  }
  // ÄNDERUNG: UFO in Magenta Farbe zeichnen mit Animation
  // Animation: unterschiedliche Helligkeit basierend auf X-Koordinate (gerade/ungerade)
  int x_coord = static_cast<int>(position[0]);
  if (x_coord % 2 == 0) {
    SDL_SetRenderDrawColor(renderer, 0xFF, 0x00, 0xFF, 0xFF);
  } else {
    SDL_SetRenderDrawColor(renderer, 0xBF, 0x00, 0xBF, 0xFF);
  }
  SDL_RenderDrawLines(renderer, points.data(), points.size());
  SDL_SetRenderDrawColor(renderer, 0xFF, 0xFF, 0xFF, 0xFF);
}
...
void SDL2Renderer::render(Torpedo * torpedo) {
  // ÄNDERUNG: Torpedo als Pluszeichen in Grün zeichnen
...
void SDL2Renderer::render(Asteroid * asteroid) {
  // ÄNDERUNG: Asteroiden als Dreiecke darstellen (einfachere Form)
  ...
  // ÄNDERUNG: Asteroiden in verschiedenen Farben je nach Größe zeichnen
  // Große Asteroiden (Größe 3): Weiß, Mittlere (Größe 2): Gelb, Kleine (Größe 1): Rot
...
void SDL2Renderer::render(SpaceshipDebris * debris) {
  // ÄNDERUNG: Wrackteile als einfache Linien darstellen
  static SDL_Point ship_points[6][2] = {{ SDL_Point{-5, -5}, SDL_Point{5, 5} }, 
                                        { SDL_Point{5, -5}, SDL_Point{-5, 5} },
                                        { SDL_Point{0, -5}, SDL_Point{0, 5} },
                                        { SDL_Point{-5, 0}, SDL_Point{5, 0} },
                                        { SDL_Point{-3, -3}, SDL_Point{3, 3} },
                                        { SDL_Point{3, -3}, SDL_Point{-3, 3} } };
  ...
  // ÄNDERUNG: Wrackteil in helles Blau zeichnen mit Fade-Out-Effekt
  for (size_t i = 0; i < debris_direction.size(); i++) {
    ...
    if ( debris->get_time_to_delete() >= 0.5 * i )  {
      // Alpha Blending simuliert durch intensitätsreduktion je älter das Trümmerteil ist
      int alpha_value = static_cast<int>(255 * debris->get_time_to_delete() / SpaceshipDebris::TIME_TO_DELETE);
      SDL_SetRenderDrawColor(renderer, 0x00, 0x7F, 0xFF, alpha_value);
...
void SDL2Renderer::render(Debris * debris) {
  // ÄNDERUNG: Explosionstrümmer als einfache Kreuze darstellen
  static SDL_Point debris_points[] = { {-5, 0}, {5, 0}, {0, -5}, {0, 5} };

  static SDL_Point point;
  Vector2df position = debris->get_position();
  // ÄNDERUNG: Explosionstrümmer in Rot zeichnen mit Expansion
  for (size_t i = 0; i < std::span{debris_points}.size(); i++) {
    ...
    // Farbe wechselt von Rot zu Orange je weiter das Trümmer expandiert
    int red_value = 255;
    int green_value = static_cast<int>(127 * (Debris::TIME_TO_DELETE - debris->get_time_to_delete()) / Debris::TIME_TO_DELETE);
    SDL_SetRenderDrawColor(renderer, red_value, green_value, 0x00, 0xFF);
...
void SDL2Renderer::renderScore() {
  ...
  do {
    ...
    x -= 20;
    // ÄNDERUNG: Score in Gelb anzeigen (Kommentare aktiviert)
    SDL_SetRenderDrawColor( renderer, 0xFF, 0xFF, 0x00, 0xFF );
